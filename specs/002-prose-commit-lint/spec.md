...
- **D5 (Markdown structural lines, owner-approved during implement):** `auto_exempts` gains `markdown-structure`, covering thematic-break lines, table delimiter rows, and bare HTML comment delimiters. These are markup structure. The pinned double-hyphen regex matched every table in the feature's own artifacts, and a gate that refuses Markdown table syntax cannot govern a table-heavy tree, so the owner chose the eighth auto-exempt over markers or an admin override (FR-004, precedence row 8).
...
- **FR-004** (ubiquitous): The gate shall exempt fenced code blocks, inline code spans, indented code, blockquote lines, URLs, file paths, shell command lines, and markdown structural lines (thematic-break lines, table delimiter rows, and bare HTML comment delimiters) from every prose rule.
